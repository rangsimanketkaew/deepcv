...
├── run_script.sh   # SLURM script for submittin a job on a cluster
└── xTB_parameters  # xTB parameter needed only you want to use extended Tight binding
```

In `MetaD.input`, specify the following input deck in `&METADYN` section.
...
where `plumed-NN.dat` is the file generated by DeepCV script.
