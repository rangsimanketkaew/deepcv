...
class Autoencoder(Model):
    ...
    def build_network(
        self, units_1, units_2, units_3, units_4, units_5,
        ...
        """
        self.units_1, self.units_2, self.units_3, self.units_4, self.units_5 = units_1, units_2, units_3, units_4, units_5
        self.func_1, self.func_2, self.func_3, self.func_4, self.func_5 = func_1, func_2, func_3, func_4, func_5
        ...
        self.latent = Dense(self.units_3, activation=self.func_3, use_bias=True)(self.encoded2)   # latent layer
        ...
        self.outputs = Dense(self.inputs.shape[1], activation=None, use_bias=True)(self.decoded2)   # reconstruct input
    ...
    def build_decoder(self, name="decoder"):
        ...
        self.decoder = Model(
            ...
            outputs=[tf.split(outputs, self.size_inputs, axis=1)],
            name=name
            )
    ...
    def train_model(self, num_epoch, batch_size, verbose=1, log_dir="./logs/"):
        ...
        if self.batch_size == 0: self.batch_size = None
...
def main():
    ...
    parser.add_argument(
        "-i", "--input", metavar="INPUT", type=str, required=True,
    ...
    if neural_network.lower() != "ae": exit(f"Error: This is an Autoencoder trainer, not {neural_network}.")
    ...
    if save_weights_npz:
        ...
        for i, layer in enumerate(model.autoencoder.layers):
            if layer.get_weights(): savez["layer" + str(i+1)] = layer.get_weights()[0]
    ...
    if save_biases_npz:
        ...
        savez = dict()
        for i, layer in enumerate(model.autoencoder.layers):
            if layer.get_weights(): savez["layer" + str(i+1)] = layer.get_weights()[1]
        np.savez_compressed(path, **savez)
